import { clsx } from "clsx";
import { Button } from "@/shared/ui/Button";
import { Wallet } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { Card } from "@/shared/ui/Card";
import { useGetBalanceQuery, useGetMeQuery } from "@/entities/user";
import { useGetMarketQuery } from "@/entities/market";
import "./InfoCard.scss";
import { useAppSelector } from "@/app/provider";

interface Props {
    setFundsModalOpen: (open: boolean) => void;
    className?: string;
}

const InfoCard = ({ setFundsModalOpen, className }: Props) => {
    const navigate = useNavigate();
    const isAuthorized = useAppSelector((state) => state.auth.isAuthenticated);
    const user = useGetMeQuery(undefined, { skip: !isAuthorized }).data;
    const userNfts = user?.nfts || [];
    const balance = useGetBalanceQuery(undefined, { skip: !isAuthorized }).data;

    const marketData = useGetMarketQuery().data;
    const marketCoins = marketData?.coins || [];
    const coinRate =
        marketCoins.find((coin) => coin.name === "SMG")?.change || 0;

    const nftsCost = userNfts.reduce((sum: number) => sum + 0, 0) || 0;

    const stats = [
        {
            title: "Balance",
            value: balance?.amount.toFixed(2),
            extra: "SMG",
            modifier: "balance",
        },
        {
            title: "Owned NFTs",
            value: user?.nfts.length ?? 0,
        },
        {
            title: "Total Value",
            value: `${nftsCost.toFixed(2)} SMG`,
            extra: `$${(nftsCost * coinRate).toFixed(2)}`,
        },
    ];

    const handleNavigateLogin = () => {
        navigate("/login");
    };

    const handleOpenModal = () => {
        setFundsModalOpen(true);
    };

    return (
        <Card className={clsx(className, "info-card")}>
            <div className="info-card__header">
                <div className="info-card__wallet-icon">
                    <Wallet width={24} height={24} />
                </div>

                <div>
                    <h3 className="info-card__title">
                        {isAuthorized ? "Your Wallet" : "Wallet Access"}
                    </h3>
                    <p className="info-card__status">
                        {isAuthorized ? "Connected" : "Not Connected"}
                    </p>
                </div>
            </div>

            {!isAuthorized ? (
                <div className="info-card__auth-warning">
                    <p className="info-card__auth-title">
                        You are not logged in
                    </p>
                    <p className="info-card__auth-subtitle">
                        Sign in to access your wallet, balance and NFTs.
                    </p>

                    <Button
                        variant="default"
                        className="info-card__login-button"
                        onClick={handleNavigateLogin}
                    >
                        Login
                    </Button>
                </div>
            ) : (
                <>
                    <div className="info-card__stats">
                        {stats.map((stat) => (
                            <div key={stat.title} className="info-card__stat">
                                <p className="info-card__stat-title primary-text">
                                    {stat.title}
                                </p>
                                <span
                                    className={clsx(
                                        "info-card__stat-value",
                                        stat.modifier &&
                                        `info-card__stat-value--${stat.modifier}`,
                                    )}
                                >
                                    {stat.value}
                                </span>
                                {stat.extra && (
                                    <p className="primary-text">{stat.extra}</p>
                                )}
                            </div>
                        ))}
                    </div>

                    <Button
                        size="sm"
                        className="info-card__add-funds"
                        onClick={handleOpenModal}
                    >
                        Add Funds
                    </Button>
                </>
            )}
        </Card>
    );
};

export { InfoCard };
